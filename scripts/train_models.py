...
def recommendation_collate_fn(
    ...
    histories, target_items, category_paths, property_ids_list, availabilities = zip(*batch)
    ...
    cat_tensor = torch.nn.utils.rnn.pad_sequence(
        category_paths, batch_first=True, padding_value=0  # type: ignore[arg-type]
    )
    prop_tensor = torch.nn.utils.rnn.pad_sequence(
        property_ids_list, batch_first=True, padding_value=0  # type: ignore[arg-type]
    )
...
def train_models(
    ...
    NUM_CATEGORIES = 1699
    NUM_PROPERTIES = 1105
    OUTPUT_DIM = 64  # Vector size shared by both towers

    # Initialize Towers and Master Model
    session_tower = SessionTower(
        vocab_size=NUM_ITEMS, embedding_dim=64, hidden_dim=64, mode="gru"
    )
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in train_loader:
            # Move raw data tensors over to the GPU
            hist, pos_item, cat, prop, avail = [tensor.to(device) for tensor in batch]
            # --- Positive Pass ---
            ...
            neg_item = torch.randint(1, NUM_ITEMS, size=pos_item.shape, device=device)
        ...
        print(
            f"Epoch {epoch + 1}/{EPOCHS} complete. Average Loss: {total_loss / len(train_loader):.4f}"
        )


if __name__ == "__main__":
    params = dvc_api.params_show(stages="train_models").get("train_models")
    if params is None:
